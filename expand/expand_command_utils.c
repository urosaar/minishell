...
/*   Updated: 2025/08/10 05:27:26 by oukhanfa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../minishell.h"

static int build_expanded_tokens(t_cmd_exp *exp, t_locals *v, int last_status, t_env **env)
{
    ...
    {
        exp->tokens[v->i] = expand_variables(v->raw_tokens[v->i], last_status, env);
        if (!exp->tokens[v->i])
